...
├── engines/           上游引擎参考源码（仅 codex；openclaw / hermes 均已打包进 runtime，不需源码）
├── bootstrap-openclaw.bat / bootstrap-hermes.bat   两引擎便携运行时引导（一次性，需联网）
├── runtime/           运行时数据（不入库）
...
关闭窗口即停止对应服务。停止全部服务用 `stop-all.bat`。首次使用需先跑 `bootstrap-openclaw.bat` 和 `bootstrap-hermes.bat`。
