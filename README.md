...
├── engines/           上游引擎源码（openclaw / codex / hermes，三引擎可切换）
│   └── hermes/bootstrap-portable.bat   Hermes 便携运行时引导（一次性）
├── runtime/           运行时数据（不入库）
...
**三个 AI 引擎**：OpenClaw（:18789，WebSocket）/ Hermes（:8642，HTTP+SSE）/ Codex（后续）。
`control-ui` 顶部可切换引擎，**实时聊天各自连自己引擎的网关、互不共用**（适配层
`control-ui/src/services/chat-engine.ts`：OpenClaw 走 WebSocket、Hermes 走 HTTP+SSE）。
Agent 由各引擎网关自行管理，Sidecar 只负责产品层能力（授权、微信桥接、Hermes 模型配置落盘）。

## Hermes 引擎

Hermes 用**项目内 vendored 源码**（`engines/hermes`）+ **便携 Python** 运行，即搬即用、不依赖外部安装：

```bash
# 1. 一次性引导（联网）：便携 Python + 依赖 → runtime/python、runtime/hermes-libs
engines\hermes\bootstrap-portable.bat
...
关闭窗口即停止对应服务。停止全部服务用 `stop-all.bat`。Hermes 首次使用需先跑 `engines\hermes\bootstrap-portable.bat`。
