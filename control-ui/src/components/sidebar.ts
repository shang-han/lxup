    ...
    return html`
      <div class="sidebar-shell">
        ...
        <div class="engine-select">
          ...
          <div class="engine-options" role="radiogroup" aria-label=${L('engine')}>
            ${this._engines.map((eng) => html`
              <label class="engine-option ${this.engine === eng.value ? 'checked' : ''}">
                <input type="radio" name="engine" value=${eng.value} .checked=${this.engine === eng.value}
                  @change=${() => this._emitEngine(eng.value)} />
                <span class="engine-option__radio" aria-hidden="true"></span>
                <span class="engine-option__text">${eng.label}</span>
              </label>
            `)}
          </div>
        </div>
