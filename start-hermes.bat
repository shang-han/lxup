...
REM  首次使用请先运行 engines\hermes\bootstrap-portable.bat。
REM ============================================================
cd /d "%~dp0"
set "ROOT=%CD%"
...
if not defined PYDIR (
  echo [错误] 未找到便携 Python，请先运行 engines\hermes\bootstrap-portable.bat
  pause
  exit /b 1
)
set "PYTHON=%PYDIR%\python.exe"
if not exist "%ROOT%\runtime\hermes-libs" (
  echo [错误] 未找到 runtime\hermes-libs，请先运行 engines\hermes\bootstrap-portable.bat
  pause
  exit /b 1
)
...
set "PYTHONPATH=%ROOT%\engines\hermes;%ROOT%\runtime\hermes-libs"
